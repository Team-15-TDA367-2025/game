...
public class TermiteBehaviour {
     ...
     * @return the target {@link CanBeAttacked} or {@code null} if there are no attack targets.
     */
        ...
        if(!potentialTargets.isEmpty()) {
            ...
            for(AttackTarget t: potentialTargets) {
                ...
                if(targetPriority.get(t.canBeAttacked.getAttackCategory()) >=  targetPriority.get(target.canBeAttacked.getAttackCategory())) {
                    // closest distance?
                    if(t.hasPosition.getPosition().dst(termite.getPosition()) < target.hasPosition.getPosition().dst(termite.getPosition())) {
                        target=t;
                    }
                }
            }
